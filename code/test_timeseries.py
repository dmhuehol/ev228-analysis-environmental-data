'''test_timeseries
Test timeseries for in-class activity in EV228 at Colorado College.
'''

from icecream import ic
import os
import sys

from matplotlib import font_manager as fm
import matplotlib.pyplot as plt
import numpy as np
import pandas as pd

import fun_import as fi
import fun_plots as fp

font_path_local = '/Users/danielhueholt/Library/Fonts/'
font_path_coe_hpc = '/home/danielhueholt/fonts/'
if os.path.exists(font_path_local):
    for font in fm.findSystemFonts(font_path_local):
        fm.fontManager.addfont(font)
elif os.path.exists(font_path_coe_hpc):
    for font in fm.findSystemFonts(font_path_coe_hpc):
        fm.fontManager.addfont(font)

#  Inputs
file_path = '/Users/danielhueholt/Data/ev228_data/station/KRDU_temp_188708-202508.csv'
fig_path = '/Users/danielhueholt/Documents/Figures/ev228_fig/20251031/'
fig_name = 'KRDU.png'
data_var = 'metANN'
time_var = 'YEAR'

df_data, df_yr = fi.import_ghcn(file_path, data_var, time_var)
ic(df_data, df_yr)
fp.timeseries(df_data, in_x=df_yr, out_path=fig_path, out_name=fig_name)